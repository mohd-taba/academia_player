...
class YPlayerController {
  ...
  Future<void> initialize(
    ...
    try {
      ...
      audioSource = audioStreamInfo.url.toString();
      // Add the audio track
      await _player
  ...
  Future<void> changeQuality(ModernPlayerVideoData data) async {
    var current = await _player.state.position;
    debugPrint("Change quality triggered.");

    await _player.stop();
    await _player.open(Media(data.source.toString()), play: false);

    await Future.delayed(const Duration(milliseconds: 500));

    // Ensure the audio track is properly loaded
    await _player
        .setAudioTrack(AudioTrack.uri(audioSource!));
  ...
  Future<void> changeVideo(ModernPlayerVideoData data) async {
    debugPrint("Change Video triggered.");

    await _player.stop();

    await Future.delayed(const Duration(milliseconds: 500));

    await initialize(data.source.toString());
